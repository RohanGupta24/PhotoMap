...
class PhotoMapViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, LocationsViewControllerDelegate {
    ...
    @IBAction func onCameraPressed(_ sender: Any) {
        ...
        if UIImagePickerController.isSourceTypeAvailable(.camera) {
            print("Camera is availalbe")
            imagePicker.sourceType = .camera
        }
        else {
            print("Camera not availabe so photo library instead")
...
extension PhotoMapViewController: MKMapViewDelegate {
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if(annotationView == nil) {
            ...
            annotationView!.leftCalloutAccessoryView = UIImageView(frame: CGRect(x:0, y:0, width: 50, heght: 50))
        }
